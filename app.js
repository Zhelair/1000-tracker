...
(() => {
  ...
  function setOptionsEnabled(enabled) {
    ...
    // We ALWAYS allow toggling the "Используем варианты" visibility (it's UI-only, not a scoring rule).
    optVariantsOn.disabled = false;

    [optRounding, optBoltsOn, optBoltsPenalty, opt555On, btnSaveOptions].forEach(el => {
      if (el) el.disabled = !enabled;
    });

    optionsHint.textContent = enabled
      ? "После 3х “Согласен ✅” настройки блокируются."
      : "Настройки заблокированы (правила согласованы).";
