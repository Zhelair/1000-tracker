...
const PLAYER_NAMES = Object.fromEntries(PLAYERS.map(p => [p.key, p.label]));
  ...
  function rulesToObj(rulesRows) {
    ...
    // Backward-compat: older builds used "to10_5up" (confusing). Treat it as "to5".
    if (o.rounding === "to10_5up") o.rounding = "to5";
  ...
  function renderAgree(state, rulesObj) {
    ...
    btnMe.onclick = async () => {
      try {
        ...
        console.error(e);
        toast("Не удалось записать согласие");
      }
    };
    ...
    gusary.addEventListener("change", async () => {
      btnAll.style.display = gusary.checked ? "" : "none";
      // If you checked it, you probably want the fanfare *now*.
      if (gusary.checked && !btnAll.disabled) {
        try {
          await btnAll.click();
        } catch (e) {
          // no-op; click handler will toast
        }
      }
    });
