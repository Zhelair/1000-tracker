...
function computeFromRounds(roundRows, rulesObj) {
    ...
    for (const r of filtered) {
      ...
      if ((barrelEvents.enter.length || barrelEvents.plus.length || barrelEvents.three.length)) {
        point.events = { barrel: barrelEvents };
        // Build a single modal payload for the last triggering round
        const items = [];
        barrelEvents.enter.forEach(k => items.push(`🛢 На бочке: ${playerLabel(k)} (>= 880)`));
        barrelEvents.plus.forEach(x => items.push(`🛢 Бочка ${x.count}/3: ${playerLabel(x.key)}`));
        barrelEvents.three.forEach(k => items.push(`💥 3 бочки: ${playerLabel(k)} — штраф −120, бочки сброшены`));
        lastEvents = { id: `barrel:${r.id || r.created_at}`, items };
      }
...
function renderScoreCards(computed, rulesObj) {
  ...
  function drawGraph(series) {
    ...
    for (let i = 0; i < series.length; i++) {
      const ev = series[i].events && series[i].events.barrel;
      if (!ev) continue;
      ...
      (ev.enter || []).forEach(pk => drawAt(pk, "🛢"));
      (ev.plus || []).forEach(x => drawAt(x.key, "🛢" + x.count));
      (ev.three || []).forEach(pk => drawAt(pk, "💥"));
    }
    ...
    try {
      ...
      if (ev && ev.id) {
        ...
        if (already !== ev.id && lastEventShownId !== ev.id) {
          ...
          if (eventTitle) eventTitle.textContent = "🛢 Бочка";
