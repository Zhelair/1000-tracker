...
(() => {
  ...
  const btnCloseSources = document.getElementById("btnCloseSources");
  ...
  const agreeGrid = document.getElementById("agreeGrid");
  const boltInputs = document.getElementById("boltInputs");
...
const PLAYER_NAMES = Object.fromEntries(PLAYERS.map(p => [p.key, p.label]));
  ...
  function setSoundButton() {
    if (!btnSound) return;
    btnSound.classList.toggle("soundOn", soundOn);
    btnSound.textContent = (soundOn ? "🔊 Звук" : "🔇 Звук");
  }
  ...
  function spawnFx(emojis, count=16){
    // FX layer lives longer than modals (so closing modal doesn't kill the vibe)
    const layer = document.createElement("div");
    layer.className = "fx-layer";
    document.body.appendChild(layer);

    const waves = 4;
    const perWave = Math.max(6, Math.floor(count / waves));
    ...
    function addWave(){
      for (let i=0;i<perWave;i++){
        ...
        const y = (Math.random()*0.25*window.innerHeight);
        const dx = (Math.random()*2-1) * (120 + Math.random()*220);
        const dy = (0.65 + Math.random()*0.45) * window.innerHeight;
        ...
        p.style.animationDuration = (1300 + Math.random()*1100) + "ms";
    ...
    addWave();
    setTimeout(addWave, 900);
    setTimeout(addWave, 1800);
    setTimeout(addWave, 2700);

    setTimeout(()=>{ try { layer.remove(); } catch(_){} }, 8000);
  }

  
  function modalFx(type){
    // visuals always ON (sound is separate)
    const card = document.querySelector("#modalEvent .modalCard") || document.querySelector("#modalWin .modalCard");
    if (card){
      card.classList.remove("fx-shake","fx-pop");
      void card.offsetWidth; // restart animation
      card.classList.add(type === "penalty" ? "fx-shake" : "fx-pop");
      setTimeout(()=>card.classList.remove("fx-shake","fx-pop"), 560);
    }

    if (type === "win") spawnFx(["🎉","✨","🏆","🎊"], 28);
    if (type === "golden") spawnFx(["✨","🌟","💫"], 22);
    if (type === "penalty") spawnFx(["💥","🔩","🛢","😵"], 22);
    if (type === "samoval") spawnFx(["🚛","💨","💨","💨"], 22);
    if (type === "gusary") spawnFx(["🎺","🐴","✅"], 18);
  }
...
function renderScoreCards(computed, rulesObj) {
  ...
  async function refresh() {
    ...
    try {
      ...
      // small SFX (no modal spam): detect increments since last refresh
      try {
        if (!prevMini) {
          prevMini = {
            bolts: { ...(computed.bolts || {}) },
            barrel_on: { ...(computed.barrel_on || {}) },
            barrel_count: { ...(computed.barrel_count || {}) },
            goldenDone: !!computed.golden_kon_completed,
          };
        } else {
          const pb = prevMini.bolts || {};
          const nb = computed.bolts || {};
          PLAYER_KEYS.forEach(k => {
            if (Number(nb[k]||0) > Number(pb[k]||0)) playSfx("boltClick");
          });

          const po = prevMini.barrel_on || {};
          const no = computed.barrel_on || {};
          const pc = prevMini.barrel_count || {};
          const nc = computed.barrel_count || {};
          PLAYER_KEYS.forEach(k => {
            if (!po[k] && !!no[k]) { playSfx("barrelEnter"); spawnFx(["🛢","💧"], 10); }
            if (Number(nc[k]||0) > Number(pc[k]||0)) playSfx("barrelTick");
          });

          if (!prevMini.goldenDone && !!computed.golden_kon_completed) { playSfx("golden"); spawnFx(["✨","⭐"], 20); }
      ...
      // Golden checkbox is informational only (auto-managed)
      try {
        const goldenActive = !!(rulesObj && rulesObj.variants_on && rulesObj.golden_on && !computed.golden_kon_completed);
        if (fGolden) {
          fGolden.checked = goldenActive;
          fGolden.disabled = true;
          fGolden.title = goldenActive ? "Золотой кон активен (авто)" : "Золотой кон не активен";
        }
      } catch (_) {}
      ...
      if (goldenHint) {
        if (rulesObj.variants_on) {
          if (!computed.golden_kon_completed) {
            ...
            goldenHint.textContent = `✨ Золотой кон: завершён (раунд #${at}) ${who} ${bidv}`;
          }
        } else {
      ...
      [fBidder, fBid, fMade, fRospis].forEach(el => { if (el) el.disabled = !canPlay; });
      if (fGolden) { fGolden.disabled = true; }
      ...
      if (ev && ev.id) {
        ...
        if (already !== ev.id && lastEventShownId !== ev.id) {
          ...
          const itemsText = (ev.items || []).join(" ");
          const isPenalty = /3 бочк|3 болт|штраф/i.test(itemsText);
          const isSam = /555→0|Самосвал 555/i.test(itemsText);
          const isGold = /Золотой кон заверш/i.test(itemsText);
          if (isPenalty) { playSfx("penalty"); modalFx("penalty"); }
          else if (isSam) { playSfx("samoval"); modalFx("samoval"); }
          else if (isGold) { playSfx("golden"); modalFx("golden"); }
          openEvent(true);
        }
      }
  ...
  btnSound && (btnSound.onclick = () => {
    soundOn = !soundOn;
    localStorage.setItem("tt_sound_on", soundOn ? "1" : "0");
    setSoundButton();
