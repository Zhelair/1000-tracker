...
// ----- Render -----
  ...
  function renderAgree(state, rulesObj) {
    ...
    btnMe.textContent = agreedSet.has(me) ? "Вы уже согласны ✅" : "Я согласен ✅";
    btnMe.disabled = !me || agreedSet.has(me);
    ...
    btnMe.onclick = async () => {
      try {
        ...
        await sb.from("rounds").insert({
          room_id: room.id,
          payload: { type: "consent", match_id: matchId, who: me, by: me, at: new Date().toISOString() }
        });
    ...
    btnAll.onclick = async () => {
      try {
        await sb.from("rounds").insert({
          room_id: room.id,
          payload: { type: "consent_all", match_id: matchId, by: me || "someone", at: new Date().toISOString() }
        });
