...
const PLAYER_NAMES = Object.fromEntries(PLAYERS.map(p => [p.key, p.label]));
  ...
  function spawnFx(emojis, count=18) {
    ...
    for (let i=0;i<count;i++){
      ...
      const y = Math.random()*H*0.12;
      const dx = (Math.random()*2-1)*W*0.25;
      const dy = H*(0.55 + Math.random()*0.35);
  ...
  function modalFx(type){
    ...
    if (type === "win") spawnFx(["🎉","✨","🏆","🎊"], 26);
    if (type === "golden") spawnFx(["✨","🌟","💫"], 18);
    if (type === "penalty") spawnFx(["💥","🔩","🛢"], 14);
    if (type === "samoval") spawnFx(["🚛","💨"], 14);
  }
  ...
  // Sound (default OFF, user can enable)
  let soundOn = (localStorage.getItem("tt_sound_on") === "1");
  const SFX = {
    win: "sounds/sound-win-cheer.mp3",
    golden: "sounds/sound-golden-complete.mp3",
    penalty: "sounds/sound-penalty-comedy.mp3",
    samoval: "sounds/sound-555-car.mp3"
  };
  const audioCache = {};
